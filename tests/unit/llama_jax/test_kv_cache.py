...
def test_length(config: ModelConfig, params: ModelParameters, n: int, token_embeddings: Array):
    ...
    # I create an empty key/value cache
    ...
    # length should be 0
    assert ll.kv_cache.length(kv_cache) == 0

    #
    # Whens
    #

    # I create a full key/value cache
    k = ll.attention.split_heads(x @ keys, config.n_kv_heads)
    v = ll.attention.split_heads(x @ values, config.n_kv_heads)
    ...
    # length should be n
    assert ll.kv_cache.length(kv_cache) == n
